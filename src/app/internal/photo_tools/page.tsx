...
export default function Page() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#F4F2F3] p-3">
      <Navbar />
      <div className="mb-2 mt-12 items-start lg:mt-10">
        <div className="flex w-full justify-between">
          <h1 className="font-serif-custom text-3xl font-black text-black">Edit Photos</h1>
        </div>
        <input 
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          className="border border-[#3D2B2E] mt-2 w-full p-2 rounded-lg text-black "
          placeholder="Search a collection"
        />
      </div>
      <div className="mt-2 overflow-hidden rounded-2xl border border-[#D8D0D3] bg-white shadow-sm">
        {displayRows.length === 0 ? (
          <div className="px-4 py-8 text-center text-sm text-[#7A6B70]">
